...
file = "Egas_Moniz_Segments/Patient6_Healthy.h5"
...
def interpolation(EMG):

    for i in EMG:
        new_EMG = np.zeros((len(EMG[i][0,:]), len(EMG[i][:,0])))
        for n in range(0,8):
            min = np.min(EMG[i][:,n])
            for idx, value in enumerate(EMG[i][:,n]):
                if value > 0.95:
                    new_EMG[idx,n] = min
            plt.plot(new_EMG[:,n])
            plt.show()

interpolation(patient1.EMG_normalization)
