...
def graph_platform(max_values, platform_COP,description, platform = False):
    ind = np.arange(2)
    width = 0.3
    error_config = {'ecolor': '1', 'capthick': 0.5}
    ...
    for i in max_values:
        ...
        if platform == True:
            ...
            plt.xlabel("Trajectory of COP on X axes", fontsize=10)
            plt.ylabel("Trajectory of COP on Y axes", fontsize=10)
            plot3.set_title("COP trajectory", fontsize=10)
        ...
        if platform == True:
            plt.subplots_adjust(top = 0.84, bottom = 0.08, left = 0.06,right=0.93, wspace=0.16, hspace=0.00)
        else:
